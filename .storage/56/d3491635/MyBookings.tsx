...
export default function MyBookings() {
  ...
  useEffect(() => {
    ...
    if (user?.userType === 'parent') {
      // For parents, show ALL bookings they created - match by multiple identifiers
      filteredBookings = storedBookings.filter((booking: Booking) => 
        // Match by contact name (from booking form)
        booking.userContact.name === user.name || 
        // Match by contact email (from booking form)
        booking.userContact.email === user.email ||
        // Match by creator ID (if available)
        booking.createdBy === user.id ||
        // Match by creator email (additional identifier)
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {user.userType === 'nanny' 
              ...
              : `All childcare bookings you've created (${bookings.length} total)`
            }
          </p>
        </div>
        ...
        {bookings.length === 0 ? (
            ...
            <CardContent>
              <div className="space-y-4">
                <h3 className="text-xl font-semibold text-gray-700">
                  {user.userType === 'nanny' ? 'No booking requests yet' : 'No bookings created yet'}
                </h3>
                <p className="text-gray-500">
                  {user.userType === 'nanny' 
                    ? 'When families book your services, their requests will appear here.'
                    : 'You haven\'t created any bookings yet. Browse our nannies to get started!'
                  }
                </p>
          ...
          <div className="space-y-6">
            {/* All Bookings for Parents, Categorized for Nannies */}
            {user.userType === 'parent' ? (
              // For parents: Show all bookings in one list, sorted by status priority
              <div>
              ...
              // For nannies: Keep the categorized view
              <>
                {/* Active Bookings */}
                <div>
