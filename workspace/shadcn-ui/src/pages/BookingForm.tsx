...
export default function BookingForm() {
  ...
  // Redirect if user is not authenticated or not a parent
  useEffect(() => {
    if (!isAuthenticated) {
      navigate('/login');
      return;
  ...
  // Don't render form if user is not a parent
  if (user?.userType !== 'parent') {
    return null;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const booking = {
      ...
      endTime: `${parseInt(formData.time.split(':')[0]) + parseInt(formData.duration)}:${formData.time.split(':')[1]}`,
      children: [], // Will be populated based on user profile later
      specialInstructions: formData.specialRequirements || undefined,
